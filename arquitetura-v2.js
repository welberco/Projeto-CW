...
async function cwRenderGalleryModal(){const item=cwGalleryItems[cwGalleryIndex];if(!item)return;el('mediaModal').style.display='flex';el('mediaContent').innerHTML='<p style="color:white">Carregando arquivo...</p>';const {data,error}=await db.storage.from('cw-anexos').createSignedUrl(item.storage_path,300);if(error)return;const url=data.signedUrl,content=item.tipo_arquivo.startsWith('image/')?`<img src="${url}" alt="${esc(item.nome_arquivo)}">`:item.tipo_arquivo.startsWith('video/')?`<video src="${url}" controls autoplay preload="metadata"></video>`:`<a class="primary" href="${url}" target="_blank" rel="noopener">Baixar ${esc(item.nome_arquivo)}</a>`;el('mediaContent').innerHTML=`<div class="cw-modal-media">${content}<div class="cw-modal-caption"><button ${cwGalleryIndex===0?'disabled':''} onclick="cwGalleryMove(-1)">←</button><span>${cwGalleryIndex+1}/${cwGalleryItems.length} · ${esc(item.nome_arquivo)}</span><button ${cwGalleryIndex===cwGalleryItems.length-1?'disabled':''} onclick="cwGalleryMove(1)">→</button></div></div>`}
