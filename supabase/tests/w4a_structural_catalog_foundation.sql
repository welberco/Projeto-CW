...
select is((select count(*) from private.authorization_profile_templates where template_key in ('manager','technician','assistant','requester') and template_version>=2),4::bigint,'all official templates retain the W4A version-two baseline or a later approved version');
select is((select count(*) from private.authorization_profile_template_permissions m join private.authorization_profile_templates t on t.id=m.template_id join public.permission_catalog p on p.id=m.permission_id where p.resource_code in ('location_types','locations','cost_centers','sectors') and t.template_key='manager'),22::bigint,'manager baseline receives exact W4A grants without use or reactivate');
select is((select count(*) from private.authorization_profile_template_permissions m join private.authorization_profile_templates t on t.id=m.template_id join public.permission_catalog p on p.id=m.permission_id where p.resource_code in ('location_types','locations','cost_centers','sectors') and t.template_key='technician'),3::bigint,'technician receives only the approved W4A lookup grants');
select is((select count(*) from private.authorization_profile_template_permissions m join private.authorization_profile_templates t on t.id=m.template_id join public.permission_catalog p on p.id=m.permission_id where p.resource_code in ('location_types','locations','cost_centers','sectors') and t.template_key='assistant'),3::bigint,'assistant receives only the approved W4A lookup grants');
select is((select count(*) from private.authorization_profile_template_permissions m join private.authorization_profile_templates t on t.id=m.template_id join public.permission_catalog p on p.id=m.permission_id where p.resource_code in ('location_types','locations','cost_centers','sectors') and t.template_key='requester'),2::bigint,'requester receives only location and sector lookup grants');
