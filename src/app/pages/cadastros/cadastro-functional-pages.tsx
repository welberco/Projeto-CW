...
function useTenantKey(resource: string, filters: Record<string, unknown> = {}) {
  ...
  return tenantQueryKey(state.projection, state.authorizationGeneration, resource, filters)
}
...
function CatalogForm({ definition, initial, locationTypes, parents, busy, submit }: { definition: Definition; initial?: CatalogRow; locationTypes: readonly StructuralCatalogLookup[]; parents: readonly StructuralCatalogLookup[]; busy: boolean; submit: (form: CommandForm) => void }) {
  ...
  return <form aria-label={initial === undefined ? `Criar ${definition.title}` : `Editar ${initial.name}`} className="mt-4 grid gap-3 rounded-lg border p-4 sm:grid-cols-2" onSubmit={(event) => { void form.handleSubmit(submit)(event) }}>
    <label className="grid gap-1 text-sm font-medium">Código<input aria-invalid={form.formState.errors.code !== undefined} className="min-h-11 rounded-md border px-3" {...form.register('code')} /><span className="text-destructive">{form.formState.errors.code?.message}</span></label>
    <label className="grid gap-1 text-sm font-medium">Nome<input aria-invalid={form.formState.errors.name !== undefined} className="min-h-11 rounded-md border px-3" {...form.register('name')} /><span className="text-destructive">{form.formState.errors.name?.message}</span></label>
    {isLocation && <label className="grid gap-1 text-sm font-medium sm:col-span-2">Tipo de Local<select aria-invalid={form.formState.errors.locationTypeId !== undefined} className="min-h-11 rounded-md border px-3" {...form.register('locationTypeId')}><option value="">Selecione</option>{locationTypes.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><span className="text-destructive">{form.formState.errors.locationTypeId?.message}</span></label>}
    {(definition === definitions.locations || definition === definitions.cost_centers) && initial === undefined && <label className="grid gap-1 text-sm font-medium sm:col-span-2">Vínculo superior<select className="min-h-11 rounded-md border px-3" {...form.register('parentId')}><option value="">Sem vínculo superior</option>{parents.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>}
    <label className="grid gap-1 text-sm font-medium sm:col-span-2">Descrição<textarea className="min-h-20 rounded-md border px-3 py-2" {...form.register('description')} /></label>
    <label className="grid gap-1 text-sm font-medium sm:col-span-2">Motivo<input aria-invalid={form.formState.errors.reason !== undefined} className="min-h-11 rounded-md border px-3" {...form.register('reason')} /><span className="text-destructive">{form.formState.errors.reason?.message}</span></label>
...
export function StructuralCatalogPage({ resource }: { resource: StructuralResource }) {
  ...
  }, onSuccess: async () => { setMessage('Alteração concluída.'); setCreating(false); setEditing(undefined); setMoving(undefined); await refresh() }, onError: async (error) => { setMessage(safeError(error)); await refresh() } })
...
function TeamForm({ initial, sectors, busy, submit }: { initial?: TeamListItem; sectors: readonly StructuralCatalogLookup[]; busy: boolean; submit: (form: CommandForm) => void }) {
  ...
  return <form aria-label={initial === undefined ? 'Criar Equipe' : `Editar ${initial.name}`} className="mt-3 grid gap-3 rounded-lg border p-4 sm:grid-cols-2" onSubmit={(event) => { void form.handleSubmit(submit)(event) }}><label className="grid gap-1 text-sm font-medium">Nome<input className="min-h-11 rounded-md border px-3" {...form.register('name')} /></label><label className="grid gap-1 text-sm font-medium">Código<input className="min-h-11 rounded-md border px-3" {...form.register('code')} /></label><label className="grid gap-1 text-sm font-medium sm:col-span-2">Setor<select className="min-h-11 rounded-md border px-3" {...form.register('sectorId')}><option value="">Sem setor</option>{sectors.map((sector) => <option key={sector.id} value={sector.id}>{sector.name}</option>)}</select></label><label className="grid gap-1 text-sm font-medium sm:col-span-2">Descrição<textarea className="min-h-20 rounded-md border px-3 py-2" {...form.register('description')} /></label><label className="grid gap-1 text-sm font-medium sm:col-span-2">Motivo<input className="min-h-11 rounded-md border px-3" {...form.register('reason')} /></label><Button disabled={busy} type="submit">{busy ? 'Salvando…' : initial === undefined ? 'Criar equipe' : 'Salvar equipe'}</Button></form>
}
...
export function TeamRosterPage() {
  const { teamId } = useParams(); const gateways = useCadastroGateways(); const { hasPermission } = useAuthorization(); const client = useQueryClient(); const rosterKey = useTenantKey('cadastros:team-roster', { teamId: teamId ?? '' })
  ...
  const candidateKey = useTenantKey('cadastros:team-candidates', { teamId: teamId ?? '', search, limit: 20, offset }); const teamListKey = useTenantKey('cadastros:teams:list', { projection: 'admin' }); const teamDetailKey = useTenantKey('cadastros:teams:detail', { teamId: teamId ?? '' })
  ...
  const refresh = () => Promise.all([client.invalidateQueries({ queryKey: rosterKey }), client.invalidateQueries({ queryKey: candidateKey }), client.invalidateQueries({ queryKey: teamListKey }), client.invalidateQueries({ queryKey: teamDetailKey })])
  const add = useMutation({ mutationFn: () => gateways!.teams.addTeamMember({ teamId: teamId!, membershipId: selected!.membership_id, ...commandContext('Inclusão de membro na equipe', 'team-member-add') }), onSuccess: async () => { setSelected(undefined); setMessage('Membro incluído.'); await refresh() }, onError: async (error) => { setMessage(safeError(error)); await refresh() } })
  const end = useMutation({ mutationFn: (member: TeamMembership) => gateways!.teams.endTeamMember({ id: member.id, expectedVersion: member.version, ...commandContext('Encerramento de vínculo na equipe', 'team-member-end') }), onSuccess: async () => { setMessage('Vínculo encerrado.'); await refresh() }, onError: async (error) => { setMessage(safeError(error)); await refresh() } })
  if (gateways === null || teamId === undefined) return <StatePanel title="Equipe indisponível" description="A equipe solicitada não está disponível." kind="error" />
